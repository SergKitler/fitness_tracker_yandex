...
func (t Training) meanSpeed() float64 {
	duration := float64(t.Duration.Minutes()) / MinInHours
...
func (r Running) Calories() float64 {
	duration := float64(r.Duration.Minutes()) / MinInHours
	return ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * duration * MinInHours)
}
...
func (r Running) TrainingInfo() InfoMessage {
	return InfoMessage{r.TrainingType, r.Duration, r.distance(), r.meanSpeed(), r.Calories()}
}
...
func (w Walking) Calories() float64 {
	...
	duration := float64(w.Duration.Minutes()) / MinInHours
	return ((CaloriesWeightMultiplier*w.Weight + meanSpeedSquared/heightInM) * CaloriesSpeedHeightMultiplier * w.Weight * duration * MinInHours)
}
...
func (w Walking) TrainingInfo() InfoMessage {
	return InfoMessage{w.TrainingType, w.Duration, w.distance(), w.meanSpeed(), w.Calories()}
}
...
func (s Swimming) Calories() float64 {
	duration := float64(s.Duration.Minutes()) / MinInHours
	return (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * duration
}
